...
export const DynamicDetail: React.FC<IDetail> = ({ featureProps, streetViewUrl }) => {
  ...
  console.log("DISORDERS ATT -----");
  console.log(disordersAttachments);
  console.log(featureProps);
  console.log(objectId);
  console.log(originalProperties?.objectid);
  console.log("^^^^");
  ...
  return (
    <div className="flex flex-col justify-end w-full gap-4">
      ...
      {disordersAttachments && !!disordersAttachments.length && (
        <div>
          <div>TU SU DOKUMENTY PRE DISORDERS_URL</div>
          <div>
            {disordersAttachments?.map((attachment, index) => {
              const attachmentUrl = `${DISORDERS_URL}/${
              ...
              return (
                <a
                  ...
                  key={index}
                >
                  {`${t("showDocument")} ${disordersAttachments.length > 1 ? index + 1 : ""}`}
                </a>
              );
            })}
          </div>
