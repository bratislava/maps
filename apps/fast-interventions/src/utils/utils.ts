...
import { ISelectOption } from "@bratislava/mapbox-maps-ui";
...
export const processData = (data: any, categories: { label: string; types: string[] }[]) => {
  ...
  const yearOptions: ISelectOption[] = uniqueYears.map((y) => ({ key: y, label: y }));
  const districtOptions: ISelectOption[] = DISTRICTS.map((d) => ({ key: d, label: d }));
...
export const getKeyStateValue = <T>(state: KeyStateRecord<T>[], key: string) => {
  const foundState = state.find((item) => item.key == key);
  return foundState ? foundState.state : undefined;
};

export const enableKeyStateValue = (state: KeyStateRecord<boolean>[], key: string) => {
  const foundState = state.find((item) => item.key == key);
  if (foundState) {
...
export const disableKeyStateValue = (state: KeyStateRecord<boolean>[], key: string) => {
  const foundState = state.find((item) => item.key == key);
  if (foundState) {
...
export const toggleKeyStateValue = (state: KeyStateRecord<boolean>[], key: string) => {
  const foundState = state.find((item) => item.key == key);
  if (foundState) {
