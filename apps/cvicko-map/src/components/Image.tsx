...
export const Image = ({ src, alt }: IImageProps) => {
  ...
  const fullUrl = useMemo(() => `/static-pages/cvicko-map/${src}`, [src]);

  return (
    <div className="relative h-40 sm:h-64 z-0">
      ...
      <img
        className={cx("w-full h-full border-0 border-transparent relative z-20 object-cover", {
          hidden: isError,
        })}
        src={fullUrl}
        alt={alt}
